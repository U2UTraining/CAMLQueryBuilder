...
namespace U2U.CamlControlLibrary
{
    public partial class WhereElement : UserControl
    {
        public WhereElement(QueryInfo query, WhereDesigner parent)
        {
            ...
            dateFunctionsComboBox.SelectedIndexChanged += async (s, e) => await AdaptUI();
        }

        private readonly QueryInfo query;

        internal WhereDesigner parent = null;

        private async Task FillValueComboBox()
        {
            if (valueComboBox.Visible && selectedField != null)
            {
                ...
                {
                    await UtilityFunctions.FillLookupComboBox(valueComboBox, query.Site, selectedField);
                }
        ...
        {
            if (fieldComboBox.SelectedItem is FieldObject field)
            {
        ...
        public string FilterValue
        {
            get
            {
                ...
                else if (selectedField.TypeAsString.Equals(DataTypes.DateTime.ToString()))
                {
                    ...
                    if (selectedIndex == 0)
                    {
                        if (int.TryParse(datePeriodsTextBox.Text, out int period) && !string.IsNullOrEmpty(datePeriodsComboBox.Text))
                        {
        ...
        {
            QueryExpressionChanged?.Invoke(this, new EventArgs());
        }
        ...
        private async void SelectedValueChanged(object sender, EventArgs e)
        {
            OnSelected();
            await AdaptUI();
