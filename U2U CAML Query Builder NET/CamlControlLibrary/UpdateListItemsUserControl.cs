...
namespace U2U.CamlControlLibrary
{
    ...
    public class UpdateListItemsUserControl : System.Windows.Forms.UserControl
    {
        ...
        private async Task InitializeControl()
        {
            ...
            foreach (var view in await query.Site.GetViews(query.List.Name))
            {
        ...
        private async void fieldsListBox_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            if (_initializing) return;
            if (fieldsListBox.SelectedIndex > -1)
            {
                ...
                if (fo.TypeAsString == U2U.SharePoint.CAML.Enumerations.DataTypes.Lookup.ToString())
                {
                    ...
                    await UtilityFunctions.FillLookupComboBox(valueComboBox, query.Site, fo);
                }
        ...
        private async void ClearQueryButtonClick(object sender, EventArgs e)
        {
            // clear the controls
